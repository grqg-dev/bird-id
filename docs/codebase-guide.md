...
birdid._write_segment_wav()    → seg_YYYYMMDD_HHMMSS.wav  (recordings/)
 ...
 │
 ├──▶ fingerprint.embed_windows() → fingerprints rows (1024-d vectors, before wav drop)
 │
...
**Constants:** `FINGERPRINT_VERSION = 1` (bump to re-fingerprint everything),
`EMBEDDING_DIM = 1024`, `SAMPLE_RATE = 48_000`, `WINDOW_SAMPLES = 144_000`.

**Functions:**

```python
embed_clip(audio_path) → np.ndarray            # first 3 s, zero-padded
embed_windows(audio_path, [(start, end), …]) → {(start, end): np.ndarray}
pack(vec) → bytes                              # little-endian float32 BLOB
unpack(blob) → np.ndarray
```

Extracts the BirdNET model's penultimate-layer features (the tensor before the
classification output). Keeps its own cached TFLite interpreter built with
`experimental_preserve_all_tensors=True` — required to read intermediate
tensors; birdnetlib's native embeddings path is broken on current TF without
it. Pure file-in/vector-out like `identifier.py`; never touches mic or DB.
TF/librosa imports are lazy.
...
| `save_fingerprint(conn, track_id, vec, dim, version)` | Upsert a track's fingerprint BLOB (caller commits) |
| `species_fingerprints(conn, name, version, min_conf)` | Fingerprinted windows for one species, oldest first |
| `fingerprint_species_counts(conn, version, min_conf)` | Per-species fingerprint counts (for `/voices` index) |
| `tracks_without_fingerprint(conn, version)` | Tracks with audio on disk missing a current-version vector |
...
fingerprints (
    ...
    vec         BLOB NOT NULL,      -- little-endian float32 × dim (4 KB)
    created_at  TEXT NOT NULL
)
...
| `/voices` | GET | Voices index: species with fingerprints + voice counts |
| `/voices/<slug>` | GET | Per-species voice clusters: hourly bars, co-singing, representative clips |
...
### Voice clustering (`/voices`)

`_compute_voices()` L2-normalizes a species' fingerprint vectors and runs
average-linkage agglomerative clustering (cosine distance, threshold
`_VOICE_DISTANCE_THRESHOLD = 0.30`, sklearn imported lazily). Only windows at
confidence ≥ `_VOICE_CONF_FLOOR = 0.5` participate; clusters smaller than
`_VOICE_MIN_MEMBERS = 3` pool into "unassigned". Voices are labeled A, B, C…
by first appearance, get a silhouette-based separation score, and report
co-singing (two voices in the same minute → very likely different birds).
Payloads cache to `recordings/cache/voices/voices_fp{N}_v{M}_{slug}.json`,
self-invalidating when the species' fingerprint count changes
(`fp_count` stored in the payload). Tuned on real data 2026-06-10; threshold
0.30 gave the best silhouettes across wren/flycatcher/hummingbird/crow.
...
fingerprint.py       wav window → 1024-d BirdNET embedding (acoustic fingerprint)
...
scripts/backfill_fingerprints.py   Embed fingerprints for tracks with audio on disk
